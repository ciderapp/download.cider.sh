...
<template>
  ...
  <div v-else>
    <b-button-group class="mr-1" v-for="file in data" :key="file">
      <b-button style="margin:5px;" variant="light" v-if="!String(file.path).endsWith('.yml') && !String(file.path).endsWith('.blockmap')">.{{ String(file.path).split('/')[String(file.path).split('/').length - 1].split('.')[String(file.path).split('/')[String(file.path).split('/').length - 1].split('.').length - 1]}}</b-button>
    </b-button-group>
  </div>
</template>
